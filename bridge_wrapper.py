...
class YOLOv7_DeepSORT:
    ...
    def track_video(self, update_text, update_image, video:str, output:str, skip_frames:int=0, show_live:bool=False, count_objects:bool=False, verbose:int = 0):
        '''
        ...
        while True: # while video is running
            ...
            global count_fps, total_sum_fps
            ...
            if verbose >= 1:
                ...
                total_sum_fps += round(fps,2)
                count_fps += 1
                
                average = total_sum_fps / count_fps
                
                update_text("DETECTION STATUS: Detection Running...", f"Processed frame no: {frame_num} || Current FPS: {round(fps,2)} || Objects tracked: {count}", f"MODEL AVG. DETECTION TIME PER FRAME: {average:.2f} FPS")
                
            result = np.asarray(frame)
            result = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
            result_rgb = cv2.cvtColor(result, cv2.COLOR_BGR2RGB)
            update_image(result_rgb)
